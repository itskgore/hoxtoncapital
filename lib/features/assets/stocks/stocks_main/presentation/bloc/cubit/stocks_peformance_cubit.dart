...
import '../../../domain/usecases/delete_stocks_usecase.dart';

class StocksPerformanceCubit extends Cubit<StocksPerformanceState> {
  final GetStocksPerformance getStocksPerformance;
  late StocksPerformanceModel performanceDate;
  ...
  StocksPerformanceCubit({
    required this.getStocksPerformance,
  }) : super(StocksPerformanceInitial());

  getStocksPerformanceData(
      {required bool merge,
      required List scope,
      ...
      String? assetType,
      String? id}) {
    emit(StocksPerformanceLoading());
    final result = getStocksPerformance({
      "merge": merge,
      'scope': scope,
      "fromDate": fromDate,
      "toDate": toDate,
    ...
    result.then((value) {
      value.fold((l) {
        if (l is TokenExpired) {
          ...
          emit(StocksPerformanceError(l.displayErrorMessage()));
        }
      }, (r) {
        var data = [];
        if (combinedGraph) {
