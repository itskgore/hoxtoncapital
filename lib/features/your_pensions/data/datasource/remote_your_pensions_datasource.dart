...
abstract class RemoteYourPension {
  Future<List<InvestmentHoldingsModel>> getHoldings(String id, String source);

  Future<PensionPerformanceModel> getPensionPerformance(
      bool merge, List scope, String fromDate, String toDate, String id);
}

class RemoteYourPensionImp extends Repository implements RemoteYourPension {
  @override
  ...
  Future<PensionPerformanceModel> getPensionPerformance(
    ...
    try {
      await isConnectedToInternet();
      log('$insightsEndpoint/aggregationResults?merge=$merge&scope=$scope&fromDate=$fromDate&toDate=$toDate&id=$id');
      final result = await Repository().dio.get(
          '$insightsEndpoint/aggregationResults?merge=$merge&scope=$scope&fromDate=$fromDate&toDate=$toDate&id=$id');
      final status = await hanldeStatusCode(result);
      if (status.status) {
        PensionPerformanceModel data =
            PensionPerformanceModel.fromJson(result.data);
        return data;
      } else {
        throw status.failure ?? const ServerFailure();
      }
